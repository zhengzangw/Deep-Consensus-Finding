...
meta = torch.load('./src/data/meta_info.pth')
DICT = meta['dict']  # {'A': 0, 'C': 1, 'G': 2, 'T': 3}
MAX_LEN = meta['max_len']  # max length of a strand; typically 120
...
class DNA_dataset(Dataset):
    ...
    def strands_to_id(self, strands, idx):
...
def get_DNA_loader(root, device='cpu', batch_size=128):
    def collate_batch(batch):
        ...
        for data, label in batch:
            data = [torch.tensor([vin['<BOS>']]), data, torch.tensor([vin['<EOS>']])]
            label = [torch.tensor([vout['<BOS>']]), label, torch.tensor([vout['<EOS>']])]
            src.append(torch.cat(data))
            tgt.append(torch.cat(label))
        src = torch.stack(src, dim=0).T.contiguous().to(device)
    ...
    dataset_info = dict(
        input_dim=len(vin), output_dim=len(vout)
    )
